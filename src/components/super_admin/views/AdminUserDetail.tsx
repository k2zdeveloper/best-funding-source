            ...
            <div className="space-y-6 animate-in slide-in-from-left-4">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-100">
                  <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2 uppercase tracking-widest text-[10px]">
                    <History className="w-4 h-4 text-blue-600" /> Platform History
                  </h3>
                  <p className="text-xs text-slate-500 mt-1">
                    {user.role === 'borrower' ? 'Active and historical loan postings.' : 'Active capital commitments and deal engagements.'}
                  </p>
                </div>

                {/* --- DYNAMIC BORROWER VIEW --- */}
                {user.role === 'borrower' ? (
                  loans && loans.length > 0 ? (
                    <div className="divide-y divide-slate-100">
                      {loans.map((loan: any) => (
                        <div 
                          key={loan.id} 
                          onClick={() => navigate('/admin-dashboard/loans', { state: { openLoanId: loan.id } })}
                        ...
                        >
                          <div>
                            <p className="font-bold text-slate-900 text-lg group-hover:text-blue-600 transition-colors">
                              {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(loan.facility_amount || 0)}
                            </p>
                            <p className="text-xs font-medium text-slate-500 mt-0.5 truncate max-w-md">
                              {loan.business_description || 'No description provided.'}
                            </p>
                            <div className="flex gap-2 mt-3">
                              <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 text-[10px] font-bold rounded border border-emerald-100">Yield: {loan.yield_rate}%</span>
                              <span className="px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-bold rounded border border-slate-200">{loan.term_length_months} Months</span>
                            </div>
                          </div>
                          <div className="flex flex-col sm:items-end gap-2">
                            <span className={`w-fit px-2.5 py-1 text-[10px] font-bold uppercase rounded-md border ${
                              loan.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                              loan.status === 'pending_review' || loan.status === 'pending' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                              'bg-slate-100 text-slate-600 border-slate-200'
                            }`}>
                              {loan.status?.replace('_', ' ')}
                            </span>
                            <div className="flex items-center gap-1 mt-1 opacity-0 group-hover:opacity-100 transition-opacity text-blue-600 text-[10px] font-bold uppercase tracking-wider">
                    ...
                    <div className="p-12 flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-sm font-bold text-slate-900">No Deal History</p>
                      <p className="text-xs text-slate-500 mt-1">This borrower has not submitted any pitches yet.</p>
                    </div>
                  )
                ) : (
                /* --- DYNAMIC LENDER VIEW --- */
                  participation && participation.length > 0 ? (
                    <div className="divide-y divide-slate-100">
                      {participation.map((part: any, idx: number) => (
                        <div 
                          key={idx} 
                          onClick={() => navigate('/admin-dashboard/loans', { state: { openLoanId: part.loan_id } })}
                          className="p-4 sm:p-6 hover:bg-slate-50 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4 group cursor-pointer"
                        >
                          <div>
                            <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-1">Capital Committed</p>
                            <p className="font-bold text-slate-900 text-lg group-hover:text-blue-600 transition-colors">
                              {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(part.amount || 0)}
                            </p>
                          </div>
                          <div className="flex flex-col sm:items-end gap-2">
                            <span className="w-fit px-2.5 py-1 text-[10px] font-bold uppercase rounded-md border bg-blue-50 text-blue-700 border-blue-200">
                              Active Commitment
                            </span>
                            <div className="flex items-center gap-1 mt-1 opacity-0 group-hover:opacity-100 transition-opacity text-blue-600 text-[10px] font-bold uppercase tracking-wider">
                              Open Deal <ArrowRight className="w-3 h-3" />
                    ...
                    <div className="p-12 flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-sm font-bold text-slate-900">No Deal Engagements</p>
                      <p className="text-xs text-slate-500 mt-1">This lender has not committed to any deals yet.</p>
                    </div>
                  )
                )}
              </div>
