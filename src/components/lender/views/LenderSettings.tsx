...
interface LenderProfileSettings {
  ...
  settings: {
    ...
    alerts: {
      new_deals: boolean;
      deal_qna_responses: boolean; // <-- Updated to match Deal Q&A
...
export const LenderSettings: React.FC<{ userData: any }> = ({ userData }) => {
  ...
  const [profile, setProfile] = useState<LenderProfileSettings>({
    ...
    settings: {
      ...
      alerts: { new_deals: true, deal_qna_responses: true, document_updates: true }
    }
  });
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      try {
        ...
        // Merge DB settings with defaults so we don't crash if a key is missing
        const dbSettings = data.settings || {};
        const dbAlerts = dbSettings.alerts || {};
  ...
  const ToggleSwitch = ({ checked, onChange }: { checked: boolean, onChange: () => void }) => (
    <button 
      type="button" 
      onClick={onChange}
      className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full focus:outline-none transition-colors duration-200 ease-in-out ${checked ? 'bg-[#1B6FA5]' : 'bg-slate-200'}`}
    >
  ...
  if (initialFetchLoading) {
    return <div className="flex justify-center py-20"><Loader2 className="w-6 h-6 animate-spin text-[#1B6FA5]" /></div>;
  }
      ...
      <form onSubmit={handleUpdateProfile} className="space-y-8">
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input 
                ...
                className="w-full bg-white border border-slate-200 px-3 py-2.5 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1B6FA5] transition-colors"
              />
            </div>
            <div>
        ...
        <section className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between gap-4 p-3 hover:bg-slate-50 rounded-lg transition-colors">
              <div>
                <span className="text-sm font-semibold text-slate-800 block">New Deals Matching Criteria</span>
                <span className="text-[10px] text-slate-500 font-medium mt-0.5 block">Get alerted when a new loan hits the marketplace in your target industry.</span>
              </div>
              <ToggleSwitch 
            ...
            <div className="flex items-center justify-between gap-4 p-3 hover:bg-slate-50 rounded-lg transition-colors border-t border-slate-100">
              <div>
                <span className="text-sm font-semibold text-slate-800 block">Deal Q&A Responses</span>
                <span className="text-[10px] text-slate-500 font-medium mt-0.5 block">Get notified when the platform admin answers your submitted diligence questions.</span>
              </div>
              <ToggleSwitch 
                checked={profile.settings.alerts.deal_qna_responses} 
                onChange={() => setProfile(p => ({...p, settings: {...p.settings, alerts: {...p.settings.alerts, deal_qna_responses: !p.settings.alerts.deal_qna_responses}}}))} 
              />
            </div>

            <div className="flex items-center justify-between gap-4 p-3 hover:bg-slate-50 rounded-lg transition-colors border-t border-slate-100">
              <div>
                <span className="text-sm font-semibold text-slate-800 block">Data Room Document Updates</span>
                <span className="text-[10px] text-slate-500 font-medium mt-0.5 block">Get alerted when a borrower uploads new financials to a deal you are tracking.</span>
              </div>
              <ToggleSwitch 
        ...
        <div className="flex justify-end pt-2">
          <button 
            ...
            className="px-8 py-3 bg-[#1B6FA5] text-white text-sm font-bold rounded-xl hover:bg-[#155A8A] transition-colors shadow-md disabled:opacity-70 flex items-center gap-2"
          >
