...
import { 
  ...
  RefreshCw, AlertTriangle, AlertCircle, ChevronRight, Briefcase, DollarSign, MessageSquareWarning
} from 'lucide-react';
...
const DashboardStatCard = ({ title, value, subtitle, icon: Icon, loading, accent, delay, onClick }: any) => {
  const accents: Record<string, string> = {
    ...
    amber: 'border-l-amber-500 text-amber-600 bg-amber-50 hover:shadow-amber-900/10',
    purple: 'border-l-purple-500 text-purple-600 bg-purple-50 hover:shadow-purple-900/10',
  };
...
const DashboardActionCard = ({ title, subtitle, icon: Icon, onClick, accent, delay }: any) => {
  const hoverBorders: Record<string, string> = {
    ...
    emerald: 'hover:border-emerald-400 hover:shadow-emerald-900/5 group-hover:bg-emerald-50 group-hover:text-emerald-600',
...
const getEntityBadge = (entity: string) => {
  ...
  if (entity.includes('verif') || entity.includes('kyc')) return 'bg-amber-50 text-amber-700 border-amber-200';
...
export const AdminOverview: React.FC = () => {
  ...
  const [metrics, setMetrics] = useState<DashboardMetrics>({ pendingKyc: 0, pendingDeals: 0, activeDeals: 0, totalVolume: 0 });
  ...
  const fetchDashboardData = useCallback(async () => {
    setError(null);
    try {
      const [borrowerReq, lenderReq, loansReq, logsReq] = await Promise.all([
        ...
        // Fetch specific fields from loans to calculate volume and counts
        supabase.from('loan_postings').select('status, facility_amount').in('status', ['pending_review', 'pending', 'active', 'fully_funded']),
      ...
      // Calculate Loan Metrics Dynamically
      const loanData = loansReq.data || [];
      const pendingDeals = loanData.filter(l => l.status === 'pending_review' || l.status === 'pending').length;
      const activeDeals = loanData.filter(l => l.status === 'active' || l.status === 'fully_funded').length;
      const totalVolume = loanData
        .filter(l => l.status === 'active' || l.status === 'fully_funded')
        .reduce((sum, loan) => sum + Number(loan.facility_amount || 0), 0);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-10 antialiased tracking-tight">
      ...
      {/* --- FINANCIAL & COMPLIANCE METRICS (4 Column Grid) --- */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <DashboardStatCard 
          ...
          subtitle="Pitches awaiting approval" 
          icon={Briefcase} 
          accent="amber" 
          delay={100} 
        ...
        <DashboardStatCard 
          ...
          value={loading ? '' : formatCurrency(metrics.totalVolume)} 
          subtitle="Total active capital" 
          icon={DollarSign} 
          accent="purple" 
          delay={300} 
          onClick={() => navigate('loans')} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both" style={{ animationDelay: '400ms' }}>
          <div className="flex items-center justify-between px-1">
            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-widest">System Activity</h3>
            <button onClick={() => navigate('reports')} className="text-[10px] font-bold text-slate-400 hover:text-blue-600 uppercase tracking-widest flex items-center gap-1 transition-all duration-300 group">
              System Audits <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
            </button>
          </div>
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  {loading ? (
                    ...
                    recentLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-slate-50/50 transition-colors duration-200 group cursor-pointer" onClick={() => navigate('reports')}>
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-bold text-slate-900 uppercase tracking-widest px-1 animate-in fade-in fill-mode-both" style={{ animationDelay: '500ms' }}>
            Operations & Control
          </h3>
          <div className="flex flex-col gap-3">
            {/* NEW: Directly links to the Deal Underwriting module */}
            <DashboardActionCard title="Underwrite Deals" subtitle="Review pending pitches" icon={Briefcase} accent="emerald" delay={500} onClick={() => navigate('loans')} />
            
            <DashboardActionCard title="Process KYC" subtitle="Approve user credentials" icon={ShieldAlert} accent="blue" delay={600} onClick={() => navigate('verifications')} />
            
            <DashboardActionCard title="Directory & Access" subtitle="Manage accounts & holds" icon={Users} accent="purple" delay={700} onClick={() => navigate('users')} />
            
            {/* UPDATED: Points to real Audits route instead of missing Support desk */}
            <DashboardActionCard title="System Audits" subtitle="Review flagged accounts" icon={MessageSquareWarning} accent="orange" delay={800} onClick={() => navigate('reports')} />
          </div>
        </div>
